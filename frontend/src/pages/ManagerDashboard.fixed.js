...
const ManagerDashboard = () => {
  ...
  const handleApproveProject = async (projectId) => {
    try {
      ...
      await api.put(`/projects/${projectId}/status`, { status: 'accepted' });
  ...
  const handleApproveExtension = async (projectId) => {
    try {
      ...
      await api.put(`/projects/${projectId}/extension`, { 
        approved: true 
      });
  ...
  const handleSubmitWork = async (e) => {
    ...
    try {
      ...
      await api.put(`/projects/${selectedProject._id}/submission`, { 
                  ...
                  {loading ? (
                    ...
                    <div className="table-responsive">
                      <table className="table table-hover table-sm align-middle">
                        ...
                        <tbody>
                          {projects.filter(project => project.status === 'accepted').length === 0 ? (
                            <tr>
                              <td colSpan="5" className="text-center py-3">
                  ...
                  {loading ? (
                    ...
                    <div className="table-responsive">
                      <table className="table table-hover table-sm align-middle">
                        ...
                        <tbody>
                          {projects.filter(project => project.status === 'deadline_extension_requested').length === 0 ? (
                            <tr>
                              <td colSpan="5" className="text-center py-3">
                                <i className="bi bi-check-circle text-muted me-2"></i>
                            ...
                            projects
                              .filter(project => project.status === 'deadline_extension_requested')
                              .map(project => (
                                <tr key={project._id}>
                                  <td>
                                    <a 
                                  ...
                                  <td>{project.extensionRequest?.newDeadline ? 
                                      formatDate(project.extensionRequest.newDeadline) :
                                      'Not specified'}
                                  </td>
                                  <td>
                                    <button
